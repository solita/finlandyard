...
//See if we can get to the villain with just one train
  ...
  if(context.knownVillainLocations.length == 1) {
      ...
      if(route!='FROMHERE') {
        ...
        firstStop=context.knownVillainLocations[0];
...
var astar = (start, goal, clockIs) => {
  ...
  const stationShortkoodit = R.map(R.prop('stationShortCode'), stations);
  ...
  const gStart=R.filter(R.propEq('name', start.stationShortCode), travelledDistances)[0];
  gStart.distance=0;
  ...
  const startPoint=R.filter(R.propEq('name', start.stationShortCode), distancesLeft)[0];
  ...
  while (openSet.length > 0) {
    ...
    const closest = R.sort(compareDistance, foundStations)[0];
    ...
    const nextStops=R.map(train => {return {train: train, stationName: dataUtils.getPossibleHoppingOffStations(train, closest.name)[0]}})(leavingTrains);
    for (var i=0; i < nextStops.length; i++) {
      const neighbor=nextStops[i];
      ...
      if(!existing) {
        const newEntry={prev: closest.name, distance: currentScore.distance, train: neighbor.train, destination: neighbor.stationName};
